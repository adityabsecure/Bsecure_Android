...
public class DraftMailsListing extends ParentFragment implements IItemHandler {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		layout = inflater.inflate(R.layout.tempalte_recyclerview, container,
				false);
		View addButton = layout.findViewById(R.id.add_button);

		addButton.setOnClickListener(bsecure);

		((TextView) layout.findViewById(R.id.catgr_txt))
				.setText(R.string.nodrafts);
	...
	public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
		...
		if (!bsecure.isDrawerOpen()) {
			...
			mSearchViewAdapter = new SearchFeedResultsAdaptor(
					this.getActivity(),
					R.layout.abc_search_dropdown_item_icons_2line, null,
					columns, null, -1000);

			autoCompleteTextView = (SearchAutoComplete) searchView
			...
			case 4:
				...
				if (results != null) {
					...
					if (object.has("status")
							&& object.optString("status").equalsIgnoreCase("0")) {
						JSONArray array = object.getJSONArray("emails_det");
						if (array != null && array.length() > 0) {
							ArrayList<String> list = new ArrayList<String>();
							for (int i = 0; i < array.length(); i++) {
								JSONObject jObject = array.getJSONObject(i);
								String useremails = jObject
										.optString("useremails");
